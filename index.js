...
/* harmony import */ var _util_convertTextToLinks__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./util/convertTextToLinks */ "./src/js/util/convertTextToLinks.js");
/* harmony import */ var _ModalCustom__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./ModalCustom */ "./src/js/ModalCustom.js");
...
<!--				</div>-->
  ...
  bindToDOM() {
    ...
    this.messagesChat = this.element.querySelector(ChatView.selectorMessagesChat);
    this.searchMessagesChat = this.element.querySelector(ChatView.selectorSearchMessagesChat);
    this.favouritesMessagesChat = this.element.querySelector(ChatView.selectorFavouritesMessagesChat);
    this.messageInput = this.element.querySelector(ChatView.selectorMessageChat);
  }
  registerEvents() {
    const chat = this.element.querySelector(ChatView.selectorChat);
    const ws = new WebSocket(`${window.hostWS}/ws`);
    ...
    this.messageInput.addEventListener('keyup', async event => {
      if (event.key === 'Enter') {
        if (event.shiftKey) {
          ...
          const text = (0,_util_convertTextToLinks__WEBPACK_IMPORTED_MODULE_2__.convertTextToLinks)((0,dompurify__WEBPACK_IMPORTED_MODULE_0__.sanitize)(this.messageInput.value.trim(), {
            ALLOWED_TAGS: ['a']
          }));
          if (!text) return;
    ...
    importBtn.addEventListener('change', async () => {
      ...
      await fetch(`${window.host}/import`, {
        method: 'POST',
        body: formData
      });
    });
    exportBtn.addEventListener('click', async () => {
      ...
      const response = await fetch(`${window.host}/export`);
    ...
    searchInput.addEventListener('input', async () => {
      if (searchInput.value.trim()) {
        const response = await fetch(`${window.host}/search/${searchInput.value}`);
        const json = await response.json();
        const {
          messages
        } = json;
    ...
    favouritesBtn.addEventListener('click', async event => {
      ...
      if (favouritesBtn.classList.contains('active')) {
        const response = await fetch(`${window.host}/favourites`);
        const json = await response.json();
        const {
          messages
        } = json;
    ...
    document.addEventListener('click', async event => {
      ...
      if (target.classList.contains('favourites-message')) {
        await fetch(`${window.host}/favourites/${target.dataset.id}`, {
          method: 'POST'
        });
      }
    });
  ...
  createMessage(message) {
    const messageObj = new _Message__WEBPACK_IMPORTED_MODULE_1__["default"](document.createElement('div'), message);
  ...
  async loadMessages() {
    const response = await fetch(`${window.host}/messages/${this.count}`);
    const {
      messages,
      noMoreData
    } = await response.json();
  ...
  async sendFiles(formData) {
    try {
      const response = await fetch(`${window.host}/upload`, {
        method: 'POST',
        body: formData
      });
  ...
  getLocation() {
    return new Promise(resolve => {
      navigator.geolocation.getCurrentPosition(data => {
        ...
        const modal = new _ModalCustom__WEBPACK_IMPORTED_MODULE_3__["default"](document.querySelector(_ModalCustom__WEBPACK_IMPORTED_MODULE_3__["default"].selectorModal));
        modal.show();
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _ChatView__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./ChatView */ "./src/js/ChatView.js");
...
const messages = new _ChatView__WEBPACK_IMPORTED_MODULE_0__["default"](app);
messages.render();

/***/ }),

/***/ "./src/js/util/convertTextToLinks.js":
/*!*******************************************!*\
  !*** ./src/js/util/convertTextToLinks.js ***!
  \*******************************************/
/***/ (function(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "convertTextToLinks": function() { return /* binding */ convertTextToLinks; }
/* harmony export */ });
